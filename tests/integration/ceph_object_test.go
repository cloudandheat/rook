...
func (s *ObjectSuite) TestWithTLS() {
	...
	runObjectE2ETest(s.helper, s.k8sh, s.Suite, s.settings.Namespace, tls, swiftAndKeystone)
	cleanUpTLS(s)
}
...
func runObjectE2ETest(helper *clients.TestClient, k8sh *utils.K8sHelper, s suite.Suite, namespace string, tlsEnable bool, swiftAndKeystone bool) {
	...
	createCephObjectStore(s.T(), helper, k8sh, namespace, storeName, 3, tlsEnable, swiftAndKeystone)
	...
	createCephObjectStore(s.T(), helper, k8sh, namespace, bucketNotificationTestStoreName, 1, tlsEnable, false)
