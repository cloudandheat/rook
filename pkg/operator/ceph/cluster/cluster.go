...
func (c *cluster) removeDefaultReplicationRule() error {
	...
	if err != nil {
		if code, ok := extractExitCode(err); ok && code == int(syscall.EBUSY) {
			...
			logger.Warning("replicated_rule is in use, not removed")
			return nil
		}
...
func (c *cluster) removeDefaultCrushMap() (err error) {
	logger.Info("removing default replicated_rule CRUSH rule for use of non-default CRUSH root")
	err = c.removeDefaultReplicationRule()
	if err != nil {
		return errors.Wrap(err, "failed to remove default rule")
	}

	logger.Info("removing default CRUSH node if applicable")
...
func (c *cluster) postMonStartupActions() error {
	...
	if config.GetCrushRootFromSpec(c.Spec) != "default" {
		...
		// internally
		if err := c.removeDefaultCrushMap(); err != nil {
